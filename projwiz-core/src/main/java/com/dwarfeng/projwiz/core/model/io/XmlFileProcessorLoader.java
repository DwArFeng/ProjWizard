...
public final class XmlFileProcessorLoader extends StreamLoader<KeySetModel<String, FileProcessor>> {
	...
	public XmlFileProcessorLoader(InputStream in, PluginClassLoader classLoader,
	 ...
	 */
	@Override
	public void load(KeySetModel<String, FileProcessor> fileProcessorModel)
			throws LoadFailedException, IllegalStateException {
		if (readFlag)
			throw new IllegalStateException("该读取器已经使用过了");

		Objects.requireNonNull(fileProcessorModel, "入口参数 fileProcessorModel 不能为 null。");
		...
		try {
			...
			for (Element claz : clazz) {
				...
				fileProcessorModel.add(FileProcessor.class.cast(object));

			}

		} catch (Exception e) {
			throw new LoadFailedException("读取文件处理器时失败。");
		}
	 ...
	 */
	@Override
	public Set<LoadFailedException> countinuousLoad(KeySetModel<String, FileProcessor> fileProcessorModel)
		...
		try {
			...
			for (Element claz : clazz) {
				try {
					...
					exceptions.add(new LoadFailedException("读取文件处理器时失败", e));
				}
			}
